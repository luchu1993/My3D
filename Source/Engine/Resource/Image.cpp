        ...
        if (fileID == "DDS ")
        {
            ...
            if (hasDXGI)
            {
                switch (dxgiHeader.dxgiFormat)
                {
                    ...
                    case DDS_DXGI_FORMAT_R8G8B8A8_UNORM_SRGB:
                        ...
                        break;
                    default:
                        MY3D_LOGERROR("Unrecognized DDS DXGI image format");
                        return false;
                }
            ...
            switch (fourCC)
            {
                ...
                case 0:
                    ...
                    {
                        MY3D_LOGERROR("Unsupported DDS pixel byte size");
                        return false;
                    }
                ...
                default:
                    MY3D_LOGERROR("Unrecognized DDS image format");
                    return false;
            }
...
#define ADJUSTSHIFT(mask, l, r) \
        ...
        else if (fileID == "\253KTX")
        {
            ...
            {
                MY3D_LOGERROR("Big-endian KTX files not supported");
                return false;
            }

            if (type != 0 || format != 0)
            {
                MY3D_LOGERROR("Uncompressed KTX files not supported");
                return false;
            }

            if (faces > 1 || depth > 1)
            {
                MY3D_LOGERROR("3D or cube KTX files not supported");
                return false;
            }

            if (mipmaps == 0)
            {
                MY3D_LOGERROR("KTX files without explicitly specified mipmap count not supported");
                return false;
            }
            ...
            {
                MY3D_LOGERROR("Unsupported texture format in KTX file");
                return false;
            }
            ...
            for (unsigned i = 0; i < mipmaps; ++i)
            {
                ...
                {
                    MY3D_LOGERROR("KTX mipmap level data size exceeds file size");
                    return false;
                }
        ...
        else if (fileID == "PVR\3")
        {
            ...
            {
                MY3D_LOGERROR("3D or cube PVR files not supported");
                return false;
            }

            if (mipmapCount == 0)
            {
                MY3D_LOGERROR("PVR files without explicitly specified mipmap count not supported");
                return false;
            }
            ...
            if (compressedFormat_ == CF_NONE)
            {
                MY3D_LOGERROR("Unsupported texture format in PVR file");
                return false;
            }
        ...
        else
        {
            ...
            if (!pixelData)
            {
                MY3D_LOGERROR("Could not load image " + source.GetName() + ": " + String(stbi_failure_reason()));
                return false;
            }
    ...
    bool Image::SetSize(int width, int height, int depth, unsigned components)
    {
        ...
        if (components > 4)
        {
            MY3D_LOGERROR("More than 4 color components are not supported");
            return false;
        }
    ...
    void Image::SetData(const unsigned char* pixelData)
    {
        ...
        if (IsCompressed())
        {
            MY3D_LOGERROR("Can not set new pixel data for a compressed image");
            return;
        }
    ...
    bool Image::LoadColorLUT(Deserializer& source)
    {
        ...
        {
            MY3D_LOGERROR("Invalid image format, can not load image");
            return false;
        }
    ...
    bool Image::FlipHorizontal()
    {
        ...
        if (depth_ > 1)
        {
            MY3D_LOGERROR("FlipHorizontal not supported for 3D images");
            return false;
        }
        ...
        else
        {
            ...
            {
                MY3D_LOGERROR("FlipHorizontal not yet implemented for other compressed formats than RGBA & DXT1,3,5");
                return false;
            }
            ...
            for (unsigned i = 0; i < numCompressedLevels_; ++i)
            {
                ...
                if (!level.data_)
                {
                    MY3D_LOGERROR("Got compressed level with no data, aborting horizontal flip");
                    return false;
                }
    ...
    bool Image::FlipVertical()
    {
        ...
        if (depth_ > 1)
        {
            MY3D_LOGERROR("FlipVertical not supported for 3D images");
            return false;
        }
        ...
        else
        {
            if (compressedFormat_ > CF_DXT5)
            {
                MY3D_LOGERROR("FlipVertical not yet implemented for other compressed formats than DXT1,3,5");
                return false;
            }
            ...
            for (unsigned i = 0; i < numCompressedLevels_; ++i)
            {
                ...
                if (!level.data_)
                {
                    MY3D_LOGERROR("Got compressed level with no data, aborting vertical flip");
                    return false;
                }
    ...
    bool Image::Resize(int width, int height)
    {
        if (IsCompressed())
        {
            MY3D_LOGERROR("Resize not supported for compressed images");
            return false;
        }

        if (depth_ > 1)
        {
            MY3D_LOGERROR("Resize not supported for 3D images");
            return false;
        }
    ...
    void Image::ClearInt(unsigned uintColor)
    {
        ...
        if (IsCompressed())
        {
            MY3D_LOGERROR("Clear not supported for compressed images");
            return;
        }
    ...
    bool Image::SaveBMP(const String& fileName) const
    {
        ...
        {
            MY3D_LOGERROR("Access denied to " + fileName);
            return false;
        }

        if (IsCompressed())
        {
            MY3D_LOGERROR("Can not save compressed image to BMP");
            return false;
        }
    ...
    bool Image::SaveTGA(const String& fileName) const
    {
        ...
        if (IsCompressed())
        {
            MY3D_LOGERROR("Can not save compressed image to TGA");
            return false;
        }
    ...
    bool Image::SaveJPG(const String& fileName, int quality) const
    {
        ...
        if (IsCompressed())
        {
            MY3D_LOGERROR("Can not save compressed image to JPG");
            return false;
        }
    ...
    bool Image::SaveDDS(const String& fileName) const
    {
        ...
        if (IsCompressed())
        {
            MY3D_LOGERROR("Can not save compressed image to DDS");
            return false;
        }

        if (components_ != 4)
        {
            MY3D_LOGERRORF("Can not save image with %u components to DDS", components_);
            return false;
        }
    ...
    SharedPtr<Image> Image::GetNextLevel() const
    {
        if (IsCompressed())
        {
            MY3D_LOGERROR("Can not generate mip level from compressed data");
            return SharedPtr<Image>();
        }
        if (components_ < 1 || components_ > 4)
        {
            MY3D_LOGERROR("Illegal number of image components for mip level generation");
            return SharedPtr<Image>();
        }
    ...
    SharedPtr<Image> Image::ConvertToRGBA() const
    {
        if (IsCompressed())
        {
            MY3D_LOGERROR("Can not convert compressed image to RGBA");
            return SharedPtr<Image>();
        }
        if (components_ < 1 || components_ > 4)
        {
            MY3D_LOGERROR("Illegal number of image components for conversion to RGBA");
            return SharedPtr<Image>();
        }
        if (!data_)
        {
            MY3D_LOGERROR("Can not convert image without data to RGBA");
            return SharedPtr<Image>();
        }
    ...
    CompressedLevel Image::GetCompressedLevel(unsigned index) const
    {
        ...
        if (compressedFormat_ == CF_NONE)
        {
            MY3D_LOGERROR("Image is not compressed");
            return level;
        }
        if (index >= numCompressedLevels_)
        {
            MY3D_LOGERROR("Compressed image mip level out of bounds");
            return level;
        }
        ...
        if (compressedFormat_ == CF_RGBA)
        {
            ...
            for (;;)
            {
                ...
                {
                    MY3D_LOGERROR("Compressed level is outside image data. Offset: " + String(offset) + " Size: " + String(level.dataSize_) +
    ...
    Image* Image::GetSubimage(const IntRect& rect) const
    {
        ...
        if (depth_ > 1)
        {
            MY3D_LOGERROR("Subimage not supported for 3D images");
            return nullptr;
        }

        if (rect.left_ < 0 || rect.top_ < 0 || rect.right_ > width_ || rect.bottom_ > height_ || !rect.Width() || !rect.Height())
        {
            MY3D_LOGERROR("Can not get subimage from image " + GetName() + " with invalid region");
            return nullptr;
        }
        ...
        else
        {
            ...
            if (!subimageLevels)
            {
                MY3D_LOGERROR("Subimage region from compressed image " + GetName() + " did not produce any data");
                return nullptr;
            }
    ...
    SDL_Surface* Image::GetSDLSurface(const IntRect& rect) const
    {
        ...
        if (depth_ > 1)
        {
            MY3D_LOGERROR("Can not get SDL surface from 3D image");
            return nullptr;
        }

        if (IsCompressed())
        {
            MY3D_LOGERROR("Can not get SDL surface from compressed image " + GetName());
            return nullptr;
        }

        if (components_ < 3)
        {
            MY3D_LOGERROR("Can not get SDL surface from image " + GetName() + " with less than 3 components");
            return nullptr;
        }
        ...
        if (surface)
        {
            ...
            MY3D_LOGERROR("Failed to create SDL surface from image " + GetName());
    ...
    bool Image::SetSubimage(const Image* image, const IntRect& rect)
    {
        ...
        {
            MY3D_LOGERROR("Image::SetSubimage is not supported for compressed or 3D images");
            return false;
        }

        if (components_ != image->components_)
        {
            MY3D_LOGERROR("Can not set subimage in image " + GetName() + " with different number of components");
            return false;
        }

        if (rect.left_ < 0 || rect.top_ < 0 || rect.right_ > width_ || rect.bottom_ > height_ || !rect.Width() || !rect.Height())
        {
            MY3D_LOGERROR("Can not set subimage in image " + GetName() + " with invalid region");
            return false;
        }
