...
namespace My3D
{
    ...
    bool ValueAnimation::LoadXML(const XMLElement &source)
    {
        ...
        XMLElement keyFramesElem = source.GetChild("keyframe");
        while (keyFramesElem)
        {
            float time = keyFramesElem.GetFloat("time");
            Variant value = keyFramesElem.GetVariant();
            SetKeyFrame(time, value);

            keyFramesElem = keyFramesElem.GetNext("keyframe");
        }
    ...
    void ValueAnimation::SetValueType(VariantType valueType)
    {
        ...
        if (valueType_ == VAR_INTRECT || valueType_ == VAR_INTVECTOR2 || valueType_ == VAR_INTVECTOR3)
        {
            ...
            // Force linear interpolation for IntRect, IntVector2, IntVector3
            if (interpolationMethod_ == IM_SPLINE)
                interpolationMethod_ = IM_LINEAR;
        }
    ...
    Variant ValueAnimation::GetAnimationValue(float scaledTime) const
    {
        ...
        {
            if (interpolatable_ == IM_LINEAR)
                return LinearInterpolation(index - 1, index, scaledTime);
            else
                return SplineInterpolation(index - 1, 1, scaledTime);
        }
    }

    void ValueAnimation::GetEventFrames(float beginTime, float endTime, PODVector<const VAnimEventFrame *>& eventFrames)
    {
        for (unsigned i = 0; i < eventFrames_.Size(); ++i)
        {
            const VAnimEventFrame& eventFrame = eventFrames_[i];
    ...
    Variant ValueAnimation::LinearInterpolation(unsigned int index1, unsigned int index2, float scaledTime) const
    {
        ...
        switch (valueType_)
        {
            ...
            default:
                MY3D_LOGERROR("Invalid value type for linear interpolation");
                return Variant::EMPTY;
        }
    }

    Variant ValueAnimation::SplineInterpolation(unsigned int index1, unsigned int index2, float scaledTime) const
    {
        ...
        switch (valueType_)
        {
            ...
            default:
                MY3D_LOGERROR("Invalid value type for spline interpolation");
                return Variant::EMPTY;
        }
    }
    ...
    Variant ValueAnimation::SubstractAndMultiply(const Variant& value1, const Variant& value2, float t) const
    {
        switch (valueType_)
        {
            ...
            default:
                MY3D_LOGERROR("Invalid value type for spline interpolation's subtract and multiply operation");
                return Variant::EMPTY;
        }
    }
}