...
bool FileSystem::SetCurrentDir(const String& pathName)
{
    ...
    {
        MY3D_LOGERROR("Access denied to " + pathName);
        return false;
    }
...
bool FileSystem::CreateDir(const String& pathName)
{
    ...
    if (success)
        MY3D_LOGDEBUG("Created directory " + pathName);
    else
        MY3D_LOGDEBUG("Failed to create directory " + pathName);
...
int FileSystem::SystemCommand(const String &commandLine, bool redirectStdOutToLog)
{
    ...
    {
        MY3D_LOGERROR("Executing an external command is not allowed");
        return -1;
    }
}
...
bool FileSystem::SystemOpen(const String &fileName, const String &mode)
{
    if (allowedPaths_.Empty())
    {
        ...
        {
            MY3D_LOGERROR("File or directory " + fileName + " not found");
            return false;
        }
...
#endif
        if (!success)
            MY3D_LOGERROR("Failed to open " + fileName + " externally");
        return success;
    }
    else
    {
        MY3D_LOGERROR("Opening a file externally is not allowed");
        return false;
    }
}
...
bool FileSystem::Copy(const String &srcFileName, const String &destFileName)
{
    ...
    {
        MY3D_LOGERROR("Access denied to " + srcFileName);
        return false;
    }
    if (!CheckAccess(GetPath(destFileName)))
    {
        MY3D_LOGERROR("Access denied to " + destFileName);
        return false;
    }
