...
AllocatorBlock* AllocatorReverseBlock(AllocatorBlock* allocator, unsigned nodeSize, unsigned capacity)
{
    if (!capacity)
        capacity = 1;
...
AllocatorBlock* AllocatorInitialize(unsigned nodeSize, unsigned initCapacity)
{
    AllocatorBlock* block = AllocatorReverseBlock(nullptr, nodeSize, initCapacity);
    return block;
}
...
void* AllocatorReserve(AllocatorBlock* allocator)
{
    ...
    if (!allocator->free_)
    {
        ...
        AllocatorReverseBlock(allocator, allocator->nodeSize_, newCapacity);
        allocator->capacity_ += newCapacity;
    }

    // Chained new free nodes
