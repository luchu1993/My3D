...
public:
    ...
    {
        return My3D::Abs(x_ * rhs.x_) + My3D::Abs(y_ * rhs.y_) + My3D::Abs(z_ * rhs.z_) + My3D::Abs(w_ * rhs.w_);
    }
    /// Project vector onto axis.
    float ProjectOntoAxis(const Vector3& axis) const { return DotProduct(Vector4(axis.Normalized(), 0.0f)); }
    /// Return absolute vector.
    Vector4 Abs() const { return Vector4(My3D::Abs(x_), My3D::Abs(y_), My3D::Abs(z_), My3D::Abs(w_)); }
    /// Linear interpolation with another vector.
    Vector4 Lerp(const Vector4& rhs, float t) const { return *this * (1.0f - t) + rhs * t; }
    /// Test for equality with another vector with epsilon.
    bool Equals(const Vector4& rhs) const
    {
        return My3D::Equals(x_, rhs.x_) && My3D::Equals(y_, rhs.y_) && My3D::Equals(z_, rhs.z_) && My3D::Equals(w_, rhs.w_);
    }
    /// Return whether any component is NaN.
    bool IsNaN() const { return My3D::IsNaN(x_) || My3D::IsNaN(y_) || My3D::IsNaN(z_) || My3D::IsNaN(w_); }
    /// Return whether any component is Inf.
    bool IsInf() const { return My3D::IsInf(x_) || My3D::IsInf(y_) || My3D::IsInf(z_) || My3D::IsInf(w_); }
    /// Convert to Vector2.
