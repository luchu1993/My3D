...
namespace My3D
{
    enum AttributeMode
    {
        /// Attribute show only in the editor, but not serialized.
        AM_EDIT = 0x0,
    ...
    MY3D_FLAGSET(AttributeMode, AttributeModeFlags);

    class Serializable;

    /// Abstract base class for invoking attribute accessors
    class MY3D_API AttributeAccessor : public RefCounted
    {
    public:
        /// Get the attribute
    ...
    struct AttributeInfo
    {
        ...
        template<typename T> T GetMetadata(const StringHash& key) const
        {
