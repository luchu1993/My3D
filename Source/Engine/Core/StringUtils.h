...
MY3D_API String GetFileSizeString(unsigned long long memorySize);
/// Decode a base64-encoded string into buffer.
MY3D_API PODVector<unsigned char> DecodeBase64(String encodedString);
/// Parse type from a C string.
template <typename T> T FromString(const char* source);
...
template <typename T> T FromString(const String& source) { return FromString<T>(source.CString()); }
}
