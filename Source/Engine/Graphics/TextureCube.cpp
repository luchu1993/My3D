...
namespace My3D
{
    ...
    bool TextureCube::SetSize(int size, unsigned format, TextureUsage usage, int multiSample)
    {
        if (size <= 0)
        {
            MY3D_LOGERROR("Zero or negative cube texture size");
            return false;
        }
        if (usage == TEXTURE_DEPTHSTENCIL)
        {
            MY3D_LOGERROR("Depth-stencil usage not supported for cube textures");
            return false;
        }
        ...
        {
            MY3D_LOGERROR("Multisampling is only supported for rendertarget cube textures");
            return false;
        }
        ...
        if (usage == TEXTURE_RENDERTARGET)
            SubscribeToEvent(E_RENDERSURFACEUPDATE, MY3D_HANDLER(TextureCube, HandleRenderSurfaceUpdate));
        else
