...
namespace My3D
{
    ...
    /// Octree octant
    class MY3D_API Octant
    {
    public:
        /// Construct
        Octant(const BoundingBox& box, unsigned level, Octant* parent, Octree* root, unsigned index = ROOT_INDEX);
        /// Destruct. Move drawable to root if available (detach if not) and free child octants.
        virtual ~Octant();
        ...
        /// Check of a drawable object fits
    ...
    class MY3D_API Octree : public Component, public Octant
    {
        MY3D_OBJECT(Octree, Component);
    public:
        /// Construct
