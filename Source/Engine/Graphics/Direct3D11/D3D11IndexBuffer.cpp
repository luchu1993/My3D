...
namespace My3D
{
    ...
    bool IndexBuffer::SetData(const void* data)
    {
        if (!data)
        {
            MY3D_LOGERROR("Null pointer for index buffer data");
            return false;
        }

        if (!indexSize_)
        {
            MY3D_LOGERROR("Index size not defined, can not set index buffer data");
            return false;
        }
    ...
    bool IndexBuffer::SetDataRange(const void* data, unsigned start, unsigned count, bool discard)
    {
        ...
        {
            MY3D_LOGERROR("Illegal range for setting new index buffer data");
            return false;
        }
    ...
    void* IndexBuffer::Lock(unsigned start, unsigned count, bool discard)
    {
        ...
        {
            MY3D_LOGERROR("Index buffer already locked");
            return nullptr;
        }

        if (!indexSize_)
        {
            MY3D_LOGERROR("Index size not defined, can not lock index buffer");
            return nullptr;
        }

        if (start + count > indexCount_)
        {
            MY3D_LOGERROR("Illegal range for locking index buffer");
            return nullptr;
        }
    ...
    bool IndexBuffer::Create()
    {
        ...
        if (graphics_)
        {
            ...
            {
                MY3D_SAFE_RELEASE(object_.ptr_);
                MY3D_LOGD3DERROR("Failed to create index buffer", hr);
                return false;
            }
        }
    ...
    void* IndexBuffer::MapBuffer(unsigned start, unsigned count, bool discard)
    {
        ...
        if (object_.ptr_)
        {
            ...
            if (FAILED(hr) || !mappedData.pData)
                MY3D_LOGD3DERROR("Failed to map index buffer", hr);
            else
            {
