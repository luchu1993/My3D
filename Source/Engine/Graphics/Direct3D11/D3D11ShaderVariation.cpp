...
namespace My3D
{
    ...
    bool ShaderVariation::LoadByteCode(const String& binaryShaderName)
    {
        ...
        {
            MY3D_LOGERROR(binaryShaderName + " is not a valid shader bytecode file");
            return false;
        }
        ...
        if (byteCodeSize)
        {
            ...
            if (type_ == VS)
                MY3D_LOGDEBUG("Loaded cached vertex shader " + GetFullName());
            else
                MY3D_LOGDEBUG("Loaded cached pixel shader " + GetFullName());
        ...
        {
            MY3D_LOGERROR(binaryShaderName + " has zero length bytecode");
            return false;
        }
    }
    ...
    bool ShaderVariation::Compile()
    {
        ...
        for (unsigned i = 0; i < defines.Size(); ++i)
        {
            ...
            if (sourceCode.Find(defines[i]) == String::NPOS)
                MY3D_LOGWARNING("Shader " + GetFullName() + " does not use the define " + defines[i]);
#endif
        }
        ...
        {
            if (type_ == VS)
                MY3D_LOGDEBUG("Compiled vertex shader " + GetFullName());
            else
                MY3D_LOGDEBUG("Compiled pixel shader " + GetFullName());
        ...
        MY3D_SAFE_RELEASE(shaderCode);
        MY3D_SAFE_RELEASE(errorMsgs);
    ...
    void ShaderVariation::ParseParameters(unsigned char* bufData, unsigned bufSize)
    {
        ...
        {
            MY3D_SAFE_RELEASE(reflection);
            MY3D_LOGD3DERROR("Failed to reflect vertex shader's input signature", hr);
            return;
        }
