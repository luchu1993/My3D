...
namespace My3D
{
    ...
    bool Texture2D::SetData(unsigned level, int x, int y, int width, int height, const void* data)
    {
        if (!object_.ptr_)
        {
            MY3D_LOGERROR("No texture created, can not set data");
            return false;
        }

        if (!data)
        {
            MY3D_LOGERROR("Null source for setting data");
            return false;
        }

        if (level >= levels_)
        {
            MY3D_LOGERROR("Illegal mip level for setting data");
            return false;
        }
        ...
        {
            MY3D_LOGERROR("Illegal dimensions for setting data");
            return false;
        }
        ...
        if (usage_ == TEXTURE_DYNAMIC)
        {
            ...
            {
                MY3D_LOGD3DERROR("Failed to map texture for update", hr);
                return false;
            }
            else
            {
    ...
    bool Texture2D::SetData(Image* image, bool useAlpha)
    {
        if (!image)
        {
            MY3D_LOGERROR("Null image, can not load texture");
            return false;
        }
    ...
    bool Texture2D::GetData(unsigned level, void* dest) const
    {
        if (!object_.ptr_)
        {
            MY3D_LOGERROR("No texture created, can not get data");
            return false;
        }

        if (!dest)
        {
            MY3D_LOGERROR("Null destination for getting data");
            return false;
        }

        if (level >= levels_)
        {
            MY3D_LOGERROR("Illegal mip level for getting data");
            return false;
        }

        if (multiSample_ > 1 && !autoResolve_)
        {
            MY3D_LOGERROR("Can not get data from multisampled texture without autoresolve");
            return false;
        }
        ...
        if (FAILED(hr))
        {
            MY3D_LOGD3DERROR("Failed to create staging texture for GetData", hr);
        ...
        if (FAILED(hr) || !mappedData.pData)
        {
            MY3D_LOGD3DERROR("Failed to map staging texture for GetData", hr);
            MY3D_SAFE_RELEASE(stagingTexture);
            return false;
        }
        else
    ...
    bool Texture2D::Create()
    {
        ...
        else if (usage_ == TEXTURE_DEPTHSTENCIL)
        {
            ...
            if (FAILED(hr))
            {
                MY3D_LOGD3DERROR("Failed to create depth-stencil view for texture", hr);
                MY3D_SAFE_RELEASE(renderSurface_->renderTargetView_);
                return false;
            }
            ...
            if (graphics_->GetImpl()->GetDevice()->GetFeatureLevel() >= D3D_FEATURE_LEVEL_11_0)
            {
                ...
                if (FAILED(hr))
                {
                    MY3D_LOGD3DERROR("Failed to create read-only depth-stencil view for texture", hr);
                    MY3D_SAFE_RELEASE(renderSurface_->readOnlyView_);
                }
            }
        }
