...
namespace My3D
{
    ...
    bool TextureCube::SetData(CubeMapFace face, Image* image, bool useAlpha)
    {
        ...
        if (!image->IsCompressed())
        {
            ...
            {
                MY3D_LOGERROR("Cube texture width not equal to height");
                return false;
            }
            ...
            else
            {
                if (!object_.ptr_)
                {
                    MY3D_LOGERROR("Cube texture face 0 must be loaded first");
                    return false;
                }
                if (levelWidth != width_ || format != format_)
                {
                    MY3D_LOGERROR("Cube texture face does not match size or format of face 0");
                    return false;
                }
            }
